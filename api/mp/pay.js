...
function setCors(res, origin) {
  ...
  res.setHeader("Access-Control-Allow-Origin", origin && ORIGINS.has(origin) ? origin : "https://clubedocavalobonfim.com.br");
...
function unwrapMpError(err) {
  ...
  const description =
    cause?.description || data?.error_description || data?.message || data?.error || err?.message || "Erro ao criar pagamento no Mercado Pago";
  const code = cause?.code || data?.status || err?.response?.status || "unknown";
  return { description, code, raw: data };
}

// SEMPRE usar e-mail sintético para cartão/boleto
function synthEmail(uid = "user") {
  const clean = String(uid).replace(/[^A-Za-z0-9]/g, "").slice(0, 20) || "user";
  return `user-${clean}@example.com`; // domínio reservado p/ testes
}
...
export default async function handler(req, res){
  ...
  try {
    ...
    if (!uid || !PLAN_MONTHS[planType]) return res.status(400).json({ error: "Parâmetros inválidos" });
    ...
    const invRef = await db.collection("users").doc(uid).collection("financeInvoices").add({
      ...
      amount, status: "em_aberto", recordedAt: FieldValue.serverTimestamp()
    });
    ...
    // nomes
    const firstName = payerForm.firstName || payerTop.firstName || payerForm.first_name || payerTop.first_name || undefined;
    const lastName  = payerForm.lastName  || payerTop.lastName  || payerForm.last_name  || payerTop.last_name  || undefined;
    const fullName  = payerForm.name || payerTop.name || undefined;

    // Tipo de pagamento
    const isCard   = payment_type_id === "credit_card" || payment_type_id === "debit_card" || (!payment_type_id && token);
    ...
    // Validações cartão
    if (isCard) {
      if (!token) return res.status(400).json({ error: "Token do cartão ausente (formData.token)." });
      if (!payment_method_id) return res.status(400).json({ error: "payment_method_id ausente (ex.: 'visa', 'master')." });
    }

    // CPF obrigatório p/ cartão e boleto
    if (isCard || isBoleto) {
      if (!isValidCPF(idNumber)) {
        if (isTestEnv) { idNumber = "19119119100"; idType = "CPF"; }
        else return res.status(400).json({ error: "CPF do pagador é obrigatório para cartão/boleto." });
      }
    }
    ...
    // E-MAIL: regra final
    // - PIX: NÃO envia email
    // - CARTÃO/BOLETO: ignora email vindo do front e SEMPRE usa sintético
    const emailToSend = isPix ? null : synthEmail(uid);
    // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if (isCard) { body.token = token; if (issuer_id) body.issuer_id = issuer_id; }
    ...
    let pay;
    try {
      pay = await mpPayment.create({ body, requestOptions: { idempotencyKey: idem } });
    } catch (err) {
      ...
      await invRef.set({ status: "erro", gatewayError: description, gatewayCode: code, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
      return res.status(500).json({
        error: description,
        code: raw?.status || code || "unknown",
        hint: isPolicy ? "Ambientes trocados ou app diferente: Access Token do servidor e Public Key do front DEVEM ser da mesma 'Credenciais de teste' do MESMO app." : null,
        debug: isTestEnv ? {
          method: isPix ? "pix" : (isCard ? "card" : (isBoleto ? "boleto" : "other")),
          sentEmail: emailToSend || "(omitted)",
          payment_method_id,
          hasToken: !!token
        } : undefined
      });
    ...
    if (approved && pay.date_approved) patch.paidAt = Timestamp.fromDate(new Date(pay.date_approved));
    ...
    // Próxima ação (PIX/BOLETO) com códigos copiáveis
    ...
    if (pay.status === "pending") {
      if (isPix) {
        ...
        next_action = {
          type: "pix",
          copy_and_paste: td?.qr_code || null,
        ...
        const barcode = td?.barcode || td?.barcode_content || poi?.barcode || poi?.qr_code || null;
        const link = td?.external_resource_url || poi?.ticket_url || null;
        next_action = { type: "boleto", barcode, link };
      }
    }

    return res.status(200).json({ id: pay.id, status: pay.status, status_detail: pay.status_detail, invoice_id: invRef.id, next_action });
  } catch (e) {
    console.error(e);
